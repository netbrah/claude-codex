...
async fn drain_to_completed(
    ...
    let mut stream = client_session
        .stream(
            ...
            // Pass the turn-level web_search_mode so a session-level Disabled is respected
            // during compaction, consistent with normal turn dispatch.
            Some(turn_context.config.web_search_mode.value()),
            // Compaction does not use custom cache retention or effort overrides.
            turn_context.config.cache_retention,
            turn_context.config.cache_retention_by_block.clone(),
            turn_context.config.model_effort,
        )
        .await?;
    loop {

...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Ok(Some(call)) => {
            ...
            if ctx
                .sess
                .record_tool_call_for_loop_detection(&call.tool_name.to_string(), &payload_preview)
                .await
            {
                tracing::warn!(
                    thread_id = %ctx.sess.conversation_id,
        ...
        Ok(None) => {
            ...
            let last_agent_message = finalized_facts.and_then(|facts| facts.last_agent_message);
            // Record assistant content for loop detection.
            if let Some(ref content) = last_agent_message
                && ctx.sess.record_content_for_loop_detection(content).await
            {
                tracing::warn!(
                    thread_id = %ctx.sess.conversation_id,

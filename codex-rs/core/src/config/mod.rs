...
pub use codex_config::types::ApprovalsReviewer;
pub use codex_config::types::SamplingParams;
pub use codex_config::types::CacheRetention;
pub use codex_config::types::CacheRetentionByBlock;
pub use codex_config::types::ModelEffort;
...
pub struct Config {
    ...
    /// Top-level `output_config.effort` default for Anthropic /messages turns.
    /// Precedence: per-turn effort > `model_effort` > provider default.
    pub model_effort: Option<ModelEffort>,

    /// Default `cache_control` retention for Anthropic /messages cache anchors.
    /// `None` means use the hard-coded default (5-minute ephemeral).
    pub cache_retention: Option<CacheRetention>,

    /// Per-anchor `cache_control` overrides. Falls back to `cache_retention`.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let config = Self {
            ...
            model_effort: cfg.model_effort,
            cache_retention: cfg.cache_retention,
            cache_retention_by_block: cfg.cache_retention_by_block,

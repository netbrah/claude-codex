...
impl ContextManager {
    ...
    /// Set the model context window in existing token_info without
    /// resetting usage counters. No-op if token_info is already populated
    /// with a context window value.
    pub(crate) fn seed_model_context_window(&mut self, context_window: i64) {
        match &mut self.token_info {
            Some(info) if info.model_context_window.is_none() => {

...
#[test]
fn seed_model_context_window_creates_info_when_none() {
    let mut cm = ContextManager::new();
    // ContextManager::new() initializes token_info as None.
    assert!(cm.token_info().is_none());

    cm.seed_model_context_window(160_000);

    let info = cm.token_info().unwrap();
    assert_eq!(info.model_context_window, Some(160_000));
    assert_eq!(info.total_token_usage, TokenUsage::default());
    assert_eq!(info.last_token_usage, TokenUsage::default());
}

#[test]
fn seed_model_context_window_is_noop_when_already_set() {
    let mut cm = ContextManager::new();
    cm.seed_model_context_window(160_000);
    // Second call with a different value should be a no-op.
    cm.seed_model_context_window(950_000);
    assert_eq!(cm.token_info().unwrap().model_context_window, Some(160_000));
}

#[test]
fn seed_model_context_window_preserves_existing_usage() {
    let mut cm = ContextManager::new();
    cm.set_token_info(Some(TokenUsageInfo {

...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        // S-022: Populate LiteLLM model catalog for non-OpenAI models.
        //
        // The bundled models.json only covers OpenAI models. Claude and other
        // TODO(XLI): re-enable LiteLLM catalog enrichment once
        // ModelsManager trait exposes refresh_litellm_catalog().
        // The old concrete ModelsManager had this method; upstream's
        // trait-based redesign removed it. XLI's litellm_model_info.rs
        // still has the implementation but needs a new integration point.
        ...
        let mut model_info = models_manager
            ...
            .await;
        // Apply backend-specific per-slug overrides (e.g. Copilot CAPI
        // caps Claude 4.6+ at 200K vs the Anthropic-direct 1M family
        // default). Without this, the TUI displays the family-default
        // context window for Copilot-hosted Claude even though CAPI
        // serves a smaller window.
        codex_provider_registry::enrich_model_info(
        ...
        // S-CTX: Seed the session's token_info with the enriched context
        // window from model resolution so the TUI /status fallback shows
        // the correct value before the first turn.
        if let Some(resolved) = model_info.resolved_context_window() {
            let effective =
                resolved.saturating_mul(model_info.effective_context_window_percent) / 100;
            let mut state = session.state.lock().await;
            state.seed_model_context_window(effective);
        }
...
impl Session {
    ...
    pub(crate) async fn record_conversation_items(
    ...
    ) {
        {
            ...
            state
                .subagent_tool_state
                .track_response_items(items.iter(), subagent_owner_from_turn_context(turn_context));
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        if turn_context.provider.info().wire_api != codex_model_provider_info::WireApi::Responses
    ...
    pub(crate) async fn build_initial_context(
        ...
        if !plan_state.is_empty() {
            use crate::context::ContextualUserFragment;
            use crate::context::PlanStateFragment;
            contextual_user_sections
                .push(PlanStateFragment::new(plan_state.to_context_string()).render());
        }
    ...
    pub(crate) async fn set_plan_state(&self, plan_state: crate::context::PlanState) {
        let mut state = self.state.lock().await;
    ...
    pub(crate) async fn record_context_updates_and_set_reference_context_item(
        ...
        // turns, plan state is already baked into build_initial_context above.
        if !should_inject_full_context && !plan_state.is_empty() {
            use crate::context::ContextualUserFragment;
            use crate::context::PlanStateFragment;
            context_items.push(ContextualUserFragment::into(PlanStateFragment::new(
                plan_state.to_context_string(),
            )));
        }
...
fn subagent_owner_from_turn_context(
    turn_context: &TurnContext,
) -> Option<crate::session::subagent_tool_state::ParentCallId> {
    use crate::session::subagent_tool_state::ParentCallId;
    use codex_protocol::protocol::{SessionSource, SubAgentSource};

    match &turn_context.session_source {
        SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            parent_spawn_call_id: Some(id),
            ..
        }) => Some(ParentCallId(id.clone())),
        _ => None,
    }
}

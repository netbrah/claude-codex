...
impl TurnContext {
    ...
    pub(crate) async fn with_model(
        ...
        let mut model_info = models_manager
            ...
            .await;
        // Apply backend-specific per-slug overrides via the provider.
        if let Err(err) = self.provider.populate_model_info(&mut model_info).await {
            tracing::warn!(
                slug = %model_info.slug,
                error = %err,
                "with_model: provider.populate_model_info failed; falling back to static metadata"
            );
        }

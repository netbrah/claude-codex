...
pub(super) async fn spawn_review_thread(
    ...
    if let Err(err) = parent_turn_context
        .provider
        .populate_model_info(&mut review_model_info)
        .await
    {
        tracing::warn!(
            slug = %review_model_info.slug,
            error = %err,
            "spawn_review_thread: provider.populate_model_info failed"
        );
    }

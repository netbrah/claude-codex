...
fn token_usage_delta(start: &TokenUsage, end: &TokenUsage) -> TokenUsage {
    TokenUsage {
        ...
        web_search_requests: (end.web_search_requests - start.web_search_requests).max(0),
        web_fetch_requests: (end.web_fetch_requests - start.web_fetch_requests).max(0),
    }
}
...
async fn run_review_on_session(
    ...
    let mut model_info = params
        ...
        .await;
    // Apply backend-specific per-slug overrides (e.g. Copilot CAPI caps).
    codex_provider_registry::enrich_model_info(
        params.parent_session.provider().await,
        Some(params.parent_session.services.auth_manager.clone()),
        &mut model_info,
    )
    .await;

...
#[test]
fn flat_index_resolves_canonical_and_variant_spellings() {
    let canonical = ToolName::namespaced("mcp__atlassian", "add_jira_comment");
    let registry = ToolRegistry::from_tools([Arc::new(TestHandler {
        tool_name: canonical.clone(),
    }) as Arc<dyn CoreToolRuntime>]);

    // Exact structured match still works.
    assert_eq!(registry.resolve_name(&canonical), Some(canonical.clone()));

    // Canonical flat form `<namespace>__<name>` arriving with namespace
    // dropped (history re-encode style) resolves back.
    let flat = ToolName::plain("mcp__atlassian__add_jira_comment");
    assert_eq!(registry.resolve_name(&flat), Some(canonical.clone()));

    // The broken delimiter-less concatenation `<namespace><name>` (the
    // `mcp__atlassianadd_jira_comment` regression) also resolves.
    let concat = ToolName::plain("mcp__atlassianadd_jira_comment");
    assert_eq!(registry.resolve_name(&concat), Some(canonical.clone()));

    // Bare tool name with namespace dropped resolves.
    let bare = ToolName::plain("add_jira_comment");
    assert_eq!(registry.resolve_name(&bare), Some(canonical));
}

#[test]
fn flat_index_refuses_ambiguous_bare_names() {
    // Two servers expose a tool of the same bare name.
    let a = ToolName::namespaced("mcp__server_a", "search");
    let b = ToolName::namespaced("mcp__server_b", "search");
    ...
    // Bare "search" is ambiguous -> refuse to guess.
    assert_eq!(registry.resolve_name(&ToolName::plain("search")), None);

    // But each fully-qualified flat form still resolves unambiguously.
    assert_eq!(
        registry.resolve_name(&ToolName::plain("mcp__server_a__search")),
        Some(a)
    );
    assert_eq!(
        registry.resolve_name(&ToolName::plain("mcp__server_b__search")),
        Some(b)
    );
}

#[test]
fn flat_index_does_not_shadow_plain_tools() {
    let shell = ToolName::plain("shell");
    let registry = ToolRegistry::from_tools([Arc::new(TestHandler {
        tool_name: shell.clone(),
    }) as Arc<dyn CoreToolRuntime>]);
    assert_eq!(registry.resolve_name(&shell), Some(shell));
    assert_eq!(registry.resolve_name(&ToolName::plain("nonexistent")), None);
}

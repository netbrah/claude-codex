...
use codex_tools::create_analyze_symbol_source_tool;
...
impl ToolExecutor<ToolInvocation> for AnalyzeSymbolSourceHandler {
    fn tool_name(&self) -> ToolName {
        ToolName::plain("analyze_symbol_source")
    }
    ...
    async fn handle(
        ...
        if let SearchStrategy::TooManyFiles { count } = search_strategy {
            return Ok(boxed_tool_output(make_too_broad_output(
                &symbol,
                &scope_path,
                count,
            )));
        ...
        if let SearchStrategy::IndexBuilding = search_strategy {
            return Ok(boxed_tool_output(make_building_output(&symbol)));
        }
...
impl CoreToolRuntime for AnalyzeSymbolSourceHandler {}

...
async fn spawn_agent_fork_context_rejects_agent_type_override() {
    ...
    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
            "Full-history forked agents inherit the parent agent type, model, and reasoning effort. To spawn with a different role/model/effort, set `fork_turns` to `\"none\"` (or a positive integer) instead of `\"all\"`, or omit `fork_turns` entirely and the spawn will default to a fresh-context fork when overrides are present.".to_string(),
        )
    );
}
...
#[tokio::test]
async fn multi_agent_v2_spawn_with_child_model_override_defaults_to_no_fork() {
    let (mut session, mut turn) = make_session_and_context().await;
    let role_name = install_role_with_model_override(&mut turn).await;
    let manager = thread_manager();
    let root = manager
    ...
    // When the caller supplies a role/model/effort override and omits `fork_turns`, the
    // v2 handler must downgrade the default from `"all"` (full-history fork, which would
    // reject the overrides) to `"none"` so the spawn actually starts. This is the natural
    // call shape mandated by XLI operator policy (`spawn_agent(agent_type="worker", ...)`).
    let output = SpawnAgentHandlerV2::default()
        ...
        .await
        .expect("child overrides with implicit fork_turns should spawn cleanly");
    let (content, _) = expect_text_output(output);
    ...
    // The role overrides must have been applied — confirming the spawn went through the
    // non-full-fork path rather than the rejected full-fork path.
...
#[tokio::test]
async fn multi_agent_v2_spawn_explicit_fork_turns_all_still_rejects_child_model_overrides() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let root = manager
        .start_thread((*turn.config).clone())
    ...
    // An *explicit* `fork_turns="all"` paired with child overrides is still a contradiction
    // and must be rejected — the caller is asking for full-history inheritance AND a
    // different model in the same breath.
    let err = SpawnAgentHandlerV2::default()
        .handle(invocation(
            Arc::new(session),
            Arc::new(turn),
            ...
            function_payload(json!({
                "message": "inspect this repo",
                "task_name": "fork_context_v2_explicit_all",
                "model": "gpt-5-child-override",
                "reasoning_effort": "low",
                "fork_turns": "all"
            })),
        ...
        .await
        .err()
        .expect("explicit fork_turns=all should reject child model overrides");

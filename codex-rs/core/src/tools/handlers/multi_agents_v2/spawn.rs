...
async fn handle_spawn_agent(
    ...
    let fork_mode = args.fork_mode(has_child_overrides)?;
    ...
    let spawn_source = thread_spawn_source(
        ...
        fork_mode.as_ref().map(|_| call_id.clone()),
    )?;
    let result = Box::pin(
...
impl SpawnAgentArgs {
    /// Resolve the requested fork mode for this spawn.
    ///
    /// When `fork_turns` is omitted, the default is `"all"` (full-history fork). However, a
    /// full-history fork must inherit the parent's role/model/effort, so if the caller also
    /// supplies any of `agent_type`, `model`, or `reasoning_effort`, omitting `fork_turns`
    /// would put us into the rejection path below. To keep the natural call shape
    /// `spawn_agent(agent_type="worker", message=...)` working (which is the documented way
    /// to bind a child to a role config), we treat an unspecified `fork_turns` as `"none"`
    /// when `has_child_overrides` is true. An *explicit* `fork_turns="all"` paired with
    /// child overrides is still passed through and rejected downstream — that combination
    /// asks for two contradictory things.
    fn fork_mode(
        &self,
        has_child_overrides: bool,
        ...
        let default_fork_turns = if has_child_overrides { "none" } else { "all" };
        let fork_turns = explicit_fork_turns.unwrap_or(default_fork_turns);

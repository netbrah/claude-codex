...
/// Extract the raw apply_patch grammar text from either a freeform
/// (`/responses`-style `ToolPayload::Custom { input }`) or a translated
/// function-call payload (`ToolPayload::Function { arguments: {"input": "..."} }`).
///
/// Why this exists: the canonical apply_patch tool is defined as a
/// freeform/grammar tool. Wires that don't have a native freeform-tool
/// concept (Anthropic `/messages`, Copilot `/chat/completions`, Gemini
/// `generateContent`) translate the freeform spec into a regular function
/// tool whose JSON schema has a single `input: string` parameter (see
/// `provider-anthropic/src/wire.rs::tools_to_anthropic_format`). When the
/// model calls it, the SSE-side parser surfaces a `ResponseItem::FunctionCall`,
/// which the router maps to `ToolPayload::Function`. Accepting both payload
/// shapes here lets a single registration serve every wire and makes
/// future upstream churn on the `Custom`/`Function` boundary non-fatal —
/// if upstream renames or refactors the freeform path, this handler still
/// receives translated tool calls from non-`/responses` wires.
///
/// Returns `Some(patch_text)` if the payload carries a usable patch
/// (either Custom input, or Function arguments with a top-level
/// `{"input": "..."}` shape). Returns `None` for any other payload kind.
/// REGRESSION GUARD: the `apply_patch_accepts_translated_function_payload`
/// test in this module fails loudly if either accept path is removed.
pub(crate) fn apply_patch_input_from_payload(payload: &ToolPayload) -> Option<String> {
    match payload {
        ToolPayload::Custom { input } => Some(input.clone()),
        ToolPayload::Function { arguments } => {
            let value: serde_json::Value = serde_json::from_str(arguments).ok()?;
            value.get("input")?.as_str().map(str::to_owned)
        }
        _ => None,
    }
}
...
/// Accepts both freeform `Custom` payloads and translated `Function` payloads
/// (see `apply_patch_input_from_payload` for the full rationale).
fn apply_patch_payload_command(payload: &ToolPayload) -> Option<String> {
    apply_patch_input_from_payload(payload)
}
...
impl ToolExecutor<ToolInvocation> for ApplyPatchHandler {
    ...
    async fn handle(
        ...
        let Some(patch_input) = apply_patch_input_from_payload(&payload) else {
...
impl CoreToolRuntime for ApplyPatchHandler {
    fn matches_kind(&self, payload: &ToolPayload) -> bool {
        // Accept freeform Custom payloads (/responses native) AND translated
        // Function payloads carrying `{"input": "..."}` (Anthropic /messages,
        // Copilot /chat/completions, Gemini generateContent). The freeform
        // tool spec is translated to a regular function tool by every wire
        // that lacks a native freeform/grammar concept — without this both-
        // shape match, every apply_patch call on non-/responses wires hits
        // the `incompatible payload` Fatal error in registry.rs.
        apply_patch_input_from_payload(payload).is_some()
    }
    ...
    fn with_updated_hook_input(
        ...
        // Rewrite in-place, preserving the inbound payload shape. Translated
        // Function payloads MUST stay Function (the wire layer expects to
        // re-emit a `tool_use` with the same arguments JSON shape on resume).

...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    add_xli_utility_tools(turn_context, planned_tools);
...
/// Register XLI custom utility tools (find/grep/read and vector search). Each
/// tool is gated to keep it off the default tool list unless explicitly
/// requested via `experimental_supported_tools` or an XLI env-var opt-in.
fn add_xli_utility_tools(turn_context: &TurnContext, planned_tools: &mut PlannedTools) {
    ...
    let env_opt_in =
        |var: &str| std::env::var(var).is_ok_and(|value| value == "1" || value == "true");

    if supports("find_files") {
        planned_tools.add(FindFilesHandler);
    }
    if supports("grep_files") {
        planned_tools.add(GrepFilesHandler);
    }
    if supports("read_file") {
        planned_tools.add(ReadFileHandler);
    }
    if supports("analyze_symbol_source") {
        planned_tools.add(AnalyzeSymbolSourceHandler);
    }

    if env_opt_in("XLI_VECTOR_SEARCH") || supports("vector_search") {
        #[cfg(feature = "vector-search")]
        {
            let cache_dir =
                dirs::home_dir().map(|home| home.join(".xli").join("cache").join("vector"));
            let model_dir = dirs::home_dir().map(|home| home.join(".xli").join("models"));
            planned_tools.add(crate::tools::handlers::VectorSearchHandler::new(
                cache_dir, model_dir,
            ));
        }
        #[cfg(not(feature = "vector-search"))]
        {
            tracing::warn!("vector_search tool requested but vector-search feature not enabled");
        }
    }
}

...
pub struct ConfigToml {
    ...
    /// Top-level Anthropic `output_config.effort` default (Anthropic /messages only).
    /// Precedence: per-turn effort override > model_effort > capability default.
    pub model_effort: Option<ModelEffort>,

    /// Default Anthropic cache_control retention (Anthropic /messages only).
    /// `"ephemeral"` uses the 5-minute TTL (default). `"1h"` enables the
    /// extended cache TTL on capable models; degrades silently on others.
    pub cache_retention: Option<CacheRetention>,

    /// Per-anchor cache_control retention overrides. Overrides cache_retention
    /// for a specific anchor; falls back to cache_retention when unset.
    #[serde(default)]
    ...
    /// resolution when spawning subagents with a different model.
    #[serde(default)]

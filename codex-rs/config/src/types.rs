...
/// Anthropic `cache_control` retention duration.
///
/// `Ephemeral` maps to `{"type": "ephemeral"}` (5-minute TTL, all models).
/// `OneHour` maps to `{"type": "ephemeral", "ttl": "1h"}` on capable models
/// (requires the `extended-cache-ttl-2025-04-11` beta header); silently
/// degrades to `Ephemeral` on models that do not advertise `supports_1h_cache`.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum CacheRetention {
    /// 5-minute prompt-cache TTL (default, all Claude models).
    #[default]
    Ephemeral,
    /// Extended 1-hour prompt-cache TTL. Requires a capable model; degrades
    /// silently on models that do not support it.
    #[serde(rename = "1h")]
    #[schemars(rename = "1h")]
    OneHour,
}

/// Per-anchor `cache_control` overrides for Anthropic prompt caching.
///
/// Each field selects a retention for one cache breakpoint. When a field is
/// `None`, the top-level `cache_retention` setting is used as the fallback.
/// Anchor names match the Anthropic convention: `system`, `tool`,
/// `user.last` (most-recent user turn), `user.previous` (penultimate turn).
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct CacheRetentionByBlock {
    /// Cache retention for the system-block anchor (last system block).
    pub system: Option<CacheRetention>,
    /// Cache retention for the tool-list anchor (last tool schema block).
    pub tool: Option<CacheRetention>,
    /// Cache retention for the most-recent user message anchor.
    #[serde(rename = "user.last")]
    #[schemars(rename = "user.last")]
    pub user_last: Option<CacheRetention>,
    /// Cache retention for the penultimate user message anchor.
    #[serde(rename = "user.previous")]
    #[schemars(rename = "user.previous")]
    pub user_previous: Option<CacheRetention>,
}

impl CacheRetentionByBlock {
    /// Returns `true` when no per-block override is set.
    pub fn is_empty(&self) -> bool {
        self.system.is_none()
            && self.tool.is_none()
            && self.user_last.is_none()
            && self.user_previous.is_none()
    }

    /// Resolve the retention for a named anchor, falling back to `default`.
    pub fn resolve(&self, anchor: CacheAnchor, default: CacheRetention) -> CacheRetention {
        let override_val = match anchor {
            CacheAnchor::System => self.system,
            CacheAnchor::Tool => self.tool,
            CacheAnchor::UserLast => self.user_last,
            CacheAnchor::UserPrevious => self.user_previous,
        };
        override_val.unwrap_or(default)
    }
}

/// Named cache anchor positions on an Anthropic `/messages` request.
...
/// Top-level `output_config.effort` default for Anthropic models.
///
/// Precedence: per-turn `effort` (from `model_reasoning_effort` or
/// `ThreadSettingsOverrides`) > `model_effort` (this field) > provider
/// capability default.

...
/// - tree      parent directories up to root looking for `./.xli/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.xli/config.toml` (loaded but disabled when untrusted)
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".xli"))
    }
}
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".xli");
        if !fs

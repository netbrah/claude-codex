...
/// Parse a flat wire tool name back into a structured `(namespace, name)`
/// pair for tool dispatch. Used by wire decoders that flatten the tool
/// name (Anthropic `/messages`, Gemini `/generateContent`).
///
/// For inputs that start with `mcp__<server>__<tool>` (the canonical
/// XLI flat wire form for namespaced MCP tools), returns
/// `(Some("mcp__<server>"), "<tool>")` so the registry HashMap lookup
/// — which keys on the same structured `ToolName` shape used at
/// registration time — succeeds.
///
/// For all other inputs (built-in tools, ad-hoc names without the
/// `mcp__` prefix, or names with no second `__` delimiter), returns
/// `(None, name)` so the call still dispatches as an unnamespaced
/// function tool.
pub fn parse_flat_mcp_tool_name(name: &str) -> (Option<String>, String) {
    let Some(rest) = name.strip_prefix(LEGACY_MCP_TOOL_NAME_PREFIX) else {
        return (None, name.to_string());
    };
    let Some(idx) = rest.find(MCP_TOOL_NAME_DELIMITER) else {
        return (None, name.to_string());
    };
    let (server, after) = rest.split_at(idx);
    let tool = &after[MCP_TOOL_NAME_DELIMITER.len()..];
    if server.is_empty() || tool.is_empty() {
        return (None, name.to_string());
    }
    (
        Some(format!("{LEGACY_MCP_TOOL_NAME_PREFIX}{server}")),
        tool.to_string(),
    )
}

#[cfg(test)]
mod parse_flat_mcp_tool_name_tests {
    use super::parse_flat_mcp_tool_name;

    #[test]
    fn parses_canonical_namespaced_form() {
        assert_eq!(
            parse_flat_mcp_tool_name("mcp__clangd_rs__analyze_symbol"),
            (
                Some("mcp__clangd_rs".to_string()),
    ...
    #[test]
    fn handles_tool_names_with_double_underscores() {
        // `mcp__server__some__deep__tool` → first `__` after `mcp__server`
        // splits namespace from name, the rest stays in the name.
        assert_eq!(
            parse_flat_mcp_tool_name("mcp__server__some__deep__tool"),
            (
                Some("mcp__server".to_string()),
                "some__deep__tool".to_string()
            )
        );
    }

    #[test]
    fn returns_unnamespaced_when_no_second_delimiter() {
        assert_eq!(
            parse_flat_mcp_tool_name("mcp__no_tool_part"),
            (None, "mcp__no_tool_part".to_string())
        );
    }

    #[test]
    fn returns_unnamespaced_for_builtin_tools() {
        assert_eq!(
            parse_flat_mcp_tool_name("shell"),
            (None, "shell".to_string())
        );
    }

    #[test]
    fn rejects_empty_server_or_tool() {
        assert_eq!(
            parse_flat_mcp_tool_name("mcp____tool"),
            (None, "mcp____tool".to_string())
        );
    }
}

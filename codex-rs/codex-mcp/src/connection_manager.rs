...
impl McpConnectionManager {
    ...
    pub async fn resolve_tool_info(&self, tool_name: &ToolName) -> Option<ToolInfo> {
        ...
        // Fast path: exact match (covers /responses native namespace).
        if let Some(hit) = all_tools
            .iter()
            .find(|tool| tool.canonical_tool_name() == *tool_name)
        {
            return Some(hit.clone());
        }
        // Fallback for wires that flatten namespace into a single name
        // string (Anthropic /messages, Gemini /generateContent —
        // neither permits `.` in tool names per their respective
        // regexes). The canonical MCP namespace is `mcp__<server>__`
        // (Display joins them with no extra separator), so the
        // flattened name on the wire is `mcp__<server>__<tool>`.
        // When the model returns a tool_use call with this flat
        // form (namespace=None), find the matching tool by checking
        // whether `name == namespace + name` for any registered tool.
        if tool_name.namespace.is_none() {
            return all_tools
                .into_iter()
                .find(|tool| tool.canonical_tool_name().to_string() == tool_name.name);
        }
        None
    }

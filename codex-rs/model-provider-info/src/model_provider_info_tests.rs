...
#[test]
fn test_wire_api_deserializes_copilot() {
    #[derive(Deserialize)]
    struct Holder {
        wire_api: WireApi,
    }
    let holder: Holder = toml::from_str("wire_api = \"copilot\"\n").unwrap();
    assert_eq!(holder.wire_api, WireApi::Copilot);
}

#[test]
fn test_wire_api_display_round_trip() {
    assert_eq!(WireApi::Copilot.to_string(), "copilot");
}

#[test]
fn test_wire_api_copilot_no_reasoning() {
    assert!(!WireApi::Copilot.supports_reasoning_effort());
    assert!(WireApi::Responses.supports_reasoning_effort());
    assert!(WireApi::Messages.supports_reasoning_effort());
}

#[test]
fn test_wire_api_chat_variant_still_rejected() {
    let err = serde_json::from_value::<WireApi>(serde_json::Value::String("chat".to_string()))
        .unwrap_err()
        .to_string();
    assert!(
        err.contains("wire_api"),
        "expected chat-removed error, got: {err}"
    );
}

#[test]
fn test_deserialize_copilot_provider_toml() {
    let provider_toml = r#"
name = "GitHub Copilot"
...
wire_api = "copilot"
        "#;
    let provider: ModelProviderInfo = toml::from_str(provider_toml).unwrap();
    assert_eq!(provider.wire_api, WireApi::Copilot);
    assert_eq!(provider.name, "GitHub Copilot");
    assert_eq!(
        provider.base_url.as_deref(),
        Some("https://api.githubcopilot.com")
    );
    // Copilot authenticates itself from ~/.config/github-copilot; no env_key
    // or auth.command needed at the provider schema level.
    assert!(provider.env_key.is_none());
    assert!(provider.auth.is_none());
    assert!(!provider.requires_openai_auth);
}

/// Regression: WireApi must round-trip through serde for every variant.
///
/// Previously the Serialize derive used `rename_all = "lowercase"`, which
/// emitted `generatecontent` for `WireApi::GenerateContent`, while the
/// hand-written Deserialize impl only accepted `generate_content`. Any code
/// path that round-tripped a WireApi (e.g. config snapshots, profile
/// projections) blew up at config-load time with:
///
///   unknown variant `generatecontent`, expected one of
///   [responses, messages, copilot, generate_content]
///
/// This test pins the contract: every variant Serializes to the exact string
/// the Deserialize impl accepts, and TOML/JSON round-trips are stable.
#[test]
fn test_wire_api_serde_round_trip_all_variants() {
    use serde_json::json;

    let cases = [
        (WireApi::Responses, "responses"),
        (WireApi::Messages, "messages"),
        (WireApi::Copilot, "copilot"),
        (WireApi::GenerateContent, "generate_content"),
    ];

    for (variant, expected) in cases {
        // Display matches the on-the-wire string.
        assert_eq!(variant.to_string(), expected, "Display for {variant:?}");

        // Serialize emits the same string the Deserialize impl accepts.
        let serialized = serde_json::to_value(variant).unwrap();
        assert_eq!(serialized, json!(expected), "Serialize for {variant:?}");

        // Round-trip via JSON.
        let back: WireApi = serde_json::from_value(serialized).unwrap();
        assert_eq!(back, variant, "JSON round-trip for {variant:?}");

        // Round-trip via TOML (the format users actually write).
        #[derive(serde::Serialize, serde::Deserialize)]
        struct Holder {
            wire_api: WireApi,
        }
        let toml_str = toml::to_string(&Holder { wire_api: variant }).unwrap();
        assert!(
            toml_str.contains(&format!("wire_api = \"{expected}\"")),
            "TOML serialize for {variant:?}: got {toml_str:?}"
        );
        let holder: Holder = toml::from_str(&toml_str).unwrap();
        assert_eq!(holder.wire_api, variant, "TOML round-trip for {variant:?}");
    }
}

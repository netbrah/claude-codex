...
pub enum WireApi {
    ...
    Messages,
    /// The GitHub Copilot chat-completions wire at
    /// `https://api.githubcopilot.com/chat/completions`. Auth, headers,
    /// 401-retry, and the `slow_down` back-off live in the
    /// `codex-copilot-adapter` crate, which delegates to `codex-copilot`
    /// (pinned from `netbrah/codex-agent`). See
    /// `docs/integration/02-design.md` in `netbrah/copilot-codex`.
    Copilot,
    /// The Google Gemini `streamGenerateContent` API.
...
impl WireApi {
    ...
    pub fn supports_reasoning_effort(&self) -> bool {
        match self {
            ...
            // Copilot does not forward reasoning params on the wire.
            Self::Copilot => false,
            // Gemini maps reasoning effort to thinkingConfig.thinkingBudget.

        ...
        self.non_empty_path(CODEX_CA_CERT_ENV)
            ...
            .or_else(|| {
                self.non_empty_path(SSL_CERT_FILE_ENV)
                    .filter(|path| !is_platform_default_ca_bundle(path))
                    .map(|path| ConfiguredCaBundle {
...
/// Returns true when `path` is a well-known platform-default CA bundle
/// that the OS TLS stack (SecureTransport / OpenSSL system store) already
/// trusts natively. Forcing rustls on these paths loses platform-specific
/// intermediate cert chaining and breaks TLS to endpoints whose chain
/// depends on AIA fetching or keychain-stored intermediates.
fn is_platform_default_ca_bundle(path: &std::path::Path) -> bool {
    // macOS system CA bundle — SecureTransport handles this natively.
    // Forcing rustls on it breaks corp proxies with intermediate-only chains.
    if path == std::path::Path::new("/etc/ssl/cert.pem") {
        return true;
    }
    // Common Linux system CA paths — these are the distro-managed bundles
    // that OpenSSL's default verify paths already cover.
    if path == std::path::Path::new("/etc/ssl/certs/ca-certificates.crt")
        || path == std::path::Path::new("/etc/pki/tls/certs/ca-bundle.crt")
    {
        return true;
    }
    false

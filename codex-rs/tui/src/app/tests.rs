...
fn side_thread_ignores_global_mcp_startup_notifications() {
    // Heavy embedded-app-server future: run on a worker thread with an enlarged
    // stack to avoid a debug-build stack overflow.
    const WORKER_THREADS: usize = 1;
    const TEST_STACK_SIZE_BYTES: usize = 8 * 1024 * 1024;
...
fn discard_side_thread_keeps_local_state_when_server_close_fails() -> Result<()> {
    // Heavy embedded-app-server future: run on a worker thread with an enlarged
    // stack to avoid a debug-build stack overflow (mirrors

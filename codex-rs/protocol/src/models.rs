...
impl InternalChatMessageMetadataPassthrough {
    // Upstream callers in `core/src/context_manager/history.rs` invoke this
    // when forwarding metadata through chat-message normalization. XLI absorbs
    // those call sites in S-WI-07 (trial upstream merge); held inert here so
    // the upstream byte-shape stays intact for the absorb.
    #[allow(dead_code)]

...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    async fn list_models(
        ...
        let client = ModelsClient::new(transport, api_provider.clone(), api_auth.clone())
            .with_telemetry(Some(request_telemetry));

        let (mut models, etag) = timeout(
        ...
        // LiteLLM enrichment: hit /v1/model/info on the same provider
        // (no-op for non-LiteLLM proxies which 404 it). Resolve the raw
        // token directly — first from CodexAuth (api-key auth mode),
        // then from the provider's env_key — instead of round-tripping
        // through to_auth_headers() + Bearer-stripping. The header path
        // silently mangles non-Bearer auth schemes (PAT bridges, SigV4,
        // etc.) and is a maintenance trap.
        let api_key = auth
            .as_ref()
            ...
            .unwrap_or_default();
        if !api_key.is_empty() {
            codex_models_manager::enrich_models_with_litellm(
                &mut models,

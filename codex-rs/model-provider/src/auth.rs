...
///
/// Public so the wrapper in `codex-core::client` can apply the same
/// external-bearer override semantics when constructing
/// `AnthropicMessagesProvider` and `CopilotModelProvider` as the
/// in-crate `create_model_provider` applies for
/// `ConfiguredModelProvider` (Sortie 1 Commit 5a).

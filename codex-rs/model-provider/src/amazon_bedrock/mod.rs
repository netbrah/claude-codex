...
///
/// Public so the `codex-provider-registry` crate can construct this
/// provider from its `create_model_provider` factory (Sortie 1
/// Commit 5a). Fields are public for the same reason — the registry
/// owns the dispatch on `WireApi`/`is_amazon_bedrock` and constructs
/// every concrete provider directly.
#[derive(Clone, Debug)]
pub struct AmazonBedrockModelProvider {
    pub info: ModelProviderInfo,
    pub aws: ModelProviderAwsAuthInfo,
}

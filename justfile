...
# Compile every test target (lib, bins, integration, benches) without running.
#
# Guards against test-bin rot (R9): a non-compiling integration test bin is
# invisible to `cargo test --lib` / nextest filters, so a guarded contract can
# silently stop being enforced — exactly what happens on an upstream merge that
# moves a symbol an integration test imports. Run this in the sortie completion
# gate. Pass `-p <crate>` to scope.
test-compile *args:
    RUST_MIN_STACK={{ rust_min_stack }} cargo test --all-targets --no-run "$@"
